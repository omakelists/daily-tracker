...
export function useAppSettings(
  ...
  const [sortUncheckedFirst, setSortUncheckedFirst] = useLocalStoragePref(
    'dt:sortUncheckedFirst',
    true,
    BOOL_PREF
  )
  const [autoDeleteExpired, setAutoDeleteExpired] = useLocalStoragePref(
    'dt:autoDeleteExpired',
    false,
    BOOL_PREF
  )
  const [autoDeleteDays, setAutoDeleteDays] = useLocalStoragePref(
    'dt:autoDeleteDays',
    1,
    INT_PREF
  )
  ...
  useEffect(() => {
    ...
    setGames((prev) =>
      prev.map((g) => {
        ...
        return filtered.length === g.items.length ?
            g
          : { ...g, items: filtered }
      })
    )
