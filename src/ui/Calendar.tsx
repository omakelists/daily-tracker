...
export function CalendarModal({ games, checks, now, onClose }: CalendarModalProps) {
      ...
      <div>
        <div className={s.filters}>
          ...
          {dailyTasks.length > 0 && (
            <select value={selTask ?? ''} onChange={(e) => setSelTask(e.target.value || null)}
              ...
              {dailyTasks.map((tk) => (
                <option key={tk.id} value={tk.id}>{tk.name.trim() || t(`types.${tk.type}`)}</option>
              ))}
            </select>
          )}
        </div>
        ...
        <div className={s.header}>
          ...
          <span className={s.month}>
            {new Date(Date.UTC(year, month, 1)).toLocaleDateString([], { year: 'numeric', month: 'long' })}
          </span>
          <button onClick={() => nav(1)} className={shared.btn}>›</button>
        </div>
