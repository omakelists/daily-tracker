...
import { markDragHandle } from './dnd.js';
...
// ── DragHandle ─────────────────────────────────────────────────────
// Sets dragFromHandle on mousedown; the parent draggable container checks this
// to ensure drags only start from this handle, not from checkboxes etc.

export function DragHandle() {
  return html`
    <div
      onMouseDown=${markDragHandle}
      style=${{
        width: L.DRAG_W, flexShrink: 0,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        color: '#3d444d', cursor: 'grab', userSelect: 'none',
...
// ── Row layout ─────────────────────────────────────────────────────
// [DRAG_W handle] [BAR_SLOT prev-bar] [CB_W checkbox] [CB_GAP] [flex content] [meta]

export function Row({ dragHandle, prevBar, checkbox, content, meta, bg, borderBottom, className, style }) {
  return html`
    <div class=${className||''} style=${{
      ...
      ${dragHandle !== undefined
        ? dragHandle
        : html`<div style=${{ width:L.DRAG_W, flexShrink:0 }} />`}
      ${prevBar}
      ${checkbox}
  ...
  return html`
    <div style=${{
      ...
      <div style=${{
        ...
        <div style=${{ fontSize:14, color:'#e6edf3', marginBottom:20, lineHeight:1.7 }}>${message}</div>
...
export function LinkButton({ url, label }) {
  if (!url) return null;
  return html`
    <a href=${url} target="_blank" rel="noopener noreferrer" title=${label||url} style=${{
      display:'flex', alignItems:'center', justifyContent:'center',
